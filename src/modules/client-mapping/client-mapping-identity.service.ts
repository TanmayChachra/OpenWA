...
/**
 * Best-effort `@lid` -> phone resolution shared by every Client Mapping write path (docs/33 Phase
 * B): the auto-tag service, "Import from Chats" (including its group-member sub-path), and any
 * future caller of {@link ClientMappingService.resolveAndUpsert}. Centralised here so the fix for
 * one duplicate-identity bug (Athar Abbas / Lakshye Kapoor — same real person, one `@c.us` row from
 * a 1:1 chat, one `@lid` row from a group participant list) can't be half-applied to some callers
 * and not others.
 *
