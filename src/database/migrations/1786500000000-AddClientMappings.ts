...
/**
 * Creates `client_mappings` — the client/teammate/group directory backing the G Brain export and
 * SLA escalation owner lookup (docs/32). `sessionId` is non-FK provenance (same reasoning as
 * `conversation_mappings`): nullable so a `kind='teammate'` row can omit it entirely, and a
 * contact/group mapping should survive session churn rather than cascade-deleting with it.
 */
