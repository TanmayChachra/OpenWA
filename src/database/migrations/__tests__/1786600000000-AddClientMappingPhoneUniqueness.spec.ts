...
describe('AddClientMappingPhoneUniqueness migration', () => {
  ...
  it('merges a pre-existing duplicate phone before the index is created (the Athar Abbas / Lakshye Kapoor bug)', async () => {
    const runner = ds.createQueryRunner();
    ...
    expect(rows[0].company).toBe('Unbundl');
    expect(rows[0].team).toBe('Performance Marketing');
