...
/**
 * docs/33 Phase C: makes "one real contact = one row per (session, phone)" a DB-level invariant,
 * not just an app-layer convention (`ClientMappingService.resolveAndUpsert`, docs/33 Phase B).
 *
 ...
 * real contact — verified live, same pushName, same effective id, different `number`). Phase B stops
 * this for every NEW row by resolving `@lid` -> phone and matching on `phone` before creating, but a
 * partial unique index is the backstop for any write path someone adds later that forgets to call
 * resolveAndUpsert — the constraint fires with a clear DB error instead of a silent duplicate.
 *
